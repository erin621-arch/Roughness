...
# ※ ズーム表示はダウンサンプリングなし（全解像度）

# ★拡大表示の設定（ここを変更して拡大範囲を調整）
zoom_width_mm  = 3.0   # Z方向（幅方向）の拡大幅 [mm]  ← 中心±half
zoom_height_mm = 0.5   # X方向（深さ方向）の拡大幅 [mm] ← 底面からこの深さ分
...
def make_base_name(f_pitch_m: float, f_depth_m: float, step: int) -> str:
    pitch_code = int(round(f_pitch_m * 1e5))
    depth_code = int(round(f_depth_m * 1e5))
    return f"kusabi_ZoomT1_pitch{pitch_code}_depth{depth_code}_step{int(step)}"
...
# =======================================================================
#               Viewer Class (Single View for Zoom T1)
# =======================================================================
class ViewerZoomSingle:
    def __init__(self, root, z_t1, time_step, output_dir,
                 extent_zoom, roi_rect_mm, base_name, mask_zoom=None):
        self.root = root

        # --- 正規化 ---
        max_t1 = np.max(np.abs(z_t1))
        if max_t1 > 1e-12:
            print(f"Normalizing Zoom T1 by max value: {max_t1:.4e}")
        ...
        self.mask_zoom   = mask_zoom   # ズーム領域に切り抜いたマスク

        os.makedirs(self.output_dir, exist_ok=True)
        self.n_frames    = self.z_t1.shape[0]
    ...
    def setup_ui(self):
        self.root.title("Kusabi Zoom T1 Viewer")
        ...
        self.fig = plt.Figure(figsize=(8, 4), dpi=100, layout="constrained")
        self.ax  = self.fig.add_subplot(111)

        vmin, vmax = -1.0, 1.0

        # Zoom T1 表示（RdBu_r: 引張/圧縮が視覚的に分かりやすい）
        self.im = self.ax.imshow(
            self.z_t1[0], cmap="RdBu_r", vmin=vmin, vmax=vmax,
            aspect="auto", interpolation="bilinear",
            extent=self.extent_zoom, zorder=1
        )
        self.ax.set_title("Zoom T1")
        self.ax.set_xlabel("Width Z (mm)")
        self.ax.set_ylabel("Depth X (mm)")

        # きず輪郭線（ズーム領域のマスクを使用）
        if self.mask_zoom is not None:
    ...
    def update_frame(self, frame_idx):
        ...
        self.im.set_array(self.z_t1[frame_idx])
    ...
    def save_mp4(self):
        ...
        def _update(i):
            self.im.set_array(self.z_t1[int(i)])
            self.time_label.config(text=f"Time: {int(i)*self.time_step*1e6:.2f} µs")
            return [self.im]

        ani = animation.FuncAnimation(self.fig, _update, frames=self.n_frames,
    ...
    def save_data(self):
        ...
        try:
            ...
            np.savez_compressed(
                ...
                mask_zoom   = self.mask_zoom if self.mask_zoom is not None else np.array([])
            )
...
# =======================================================================
def main():
    base_name = make_base_name(f_pitch, f_depth, step_size)

    print(f"Pitch(p)    = {f_pitch*1000} mm")
    print(f"Depth(d)    = {f_depth*1000} mm")
    print(f"Step Size   = {step_size} mesh(es)")
    print(f"Zoom region : Z[{zoom_z_min:.2f}, {zoom_z_max:.2f}] mm  "
          f"X[{zoom_x_min:.2f}, {zoom_x_max:.2f}] mm")
    ...
    # --- マスク生成 ---
    T13_isfree_np, T5_isfree_np = isfree_kusabi(
        nx, ny, f_pitch, f_depth, mesh_length, step_size)
    ...
    # ズーム領域のマスク（x方向: x_start:x_end, z方向: y_start:y_end）
    mask_zoom = T13_isfree_np[x_start:x_end, y_start:y_end]
    ...
    for t in range(int(t_max)):
        ...
        T1[1:nx, :] -= dtx * (c11 * (Ux[1:nx, :] - Ux[0:nx-1, :])
                               + c13 * (Uy[1:nx, 1:] - Uy[1:nx, :-1]))
        T3[1:nx, :] -= dtx * (c13 * (Ux[1:nx, :] - Ux[0:nx-1, :])
                               + c11 * (Uy[1:nx, 1:] - Uy[1:nx, :-1]))
        T5[:, 1:ny] -= dtx * c55 * (Ux[:, 1:] - Ux[:, :-1]
                                     + Uy[1:, 1:ny] - Uy[:-1, 1:ny])
