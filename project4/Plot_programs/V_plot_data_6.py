...
#  形状ごとの処理 (単体スペクトル版)
# ============================================================
def process_shape_metrics_single(shape_name):
    print(f"\n[{shape_name}] 単体スペクトル評価 (Defect Only) ...")
    ...
    exp_norm = exp_spec_tgt / np.max(exp_spec_tgt)
    
    # --- 2. シミュレーション Step Loop ---
    results = []
    ...
    for step in STEP_LIST:
        ...
        try:
            ...
            sim_norm = sim_spec_tgt / np.max(sim_spec_tgt)
            
            # 指標計算 (相関係数削除)
            mae, rmse = calculate_metrics(exp_norm, sim_norm)
            results.append((step, mae, rmse))
            
            # print(f"  Step {step}: Corr={corr:.3f}") # 削除
    ...
    # MAE
    axes[0].plot(steps, maes, 'o-', color='tab:blue')
    ...
    # RMSE
    axes[1].plot(steps, rmses, 's-', color='tab:orange')
    ...
    fig.suptitle(f"Evaluation Step Size : {shape_name} P{TARGET_PITCH_NAME} D{TARGET_DEPTH_NAME}", fontsize=16)
    plt.tight_layout()
    
    save_name = f"6_Step_Metrics_{shape_name}.png"
    plt.savefig(save_name, dpi=150)
    print(f"  -> Saved: {save_name}")
    plt.close(fig)

    # まとめてプロットするためにデータを返す
    return steps, maes, rmses
...
def plot_combined_metrics(all_data):
    ...
    fig.suptitle(f"Step Size Evaluation - All Shapes (P{TARGET_PITCH_NAME} D{TARGET_DEPTH_NAME})", fontsize=16)
    plt.tight_layout()
    
    save_name = "6_Step_Metrics_All_Shapes.png"
    plt.savefig(save_name, dpi=150)
    print(f"  -> Saved: {save_name}")
    plt.close(fig)
...
# ============================================================
def main():
    print("評価指標の推移グラフを作成します...")
    
    all_data = {} # 全形状データを格納する辞書

    for shape in TARGET_SHAPES:
        result = process_shape_metrics_single(shape)
