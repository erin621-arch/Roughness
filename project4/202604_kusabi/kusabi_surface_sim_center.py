...
# 外側の凸角：段が上がる位置 z_{i+1} に、前の段（深い側）の x 座標を対応
_corner_z = []
_corner_x = []
...
_meas_z = np.array(_corner_z, dtype=int)   # 凸角のみ
...
# T1: z=_z_rec_end-1 における x 方向断面マップ（x = nx-mn_d 〜 nx）
_z_obs       = _z_rec_end - 1                   # 観測 z（ピッチ境界の1メッシュ手前・固体）
_x_obs_start = nx - mn_d                         # f_depth ライン
_x_obs_end   = nx                                # 底面ライン
_T1_xslice   = np.zeros((_x_obs_end - _x_obs_start + 1, _t_rec_len), dtype=float)
...
print(f"Probe center sz = {sz}  (pitch center)")
...
_npz_path = os.path.join(
    output_dir,
    f"kusabi_surface_map_center_pitch{int(f_pitch*1e5)}_depth{int(f_depth*1e5)}.npz"
)
np.savez(
    _npz_path,
