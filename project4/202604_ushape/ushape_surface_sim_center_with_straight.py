...
# ---- [変更①] T1 アーク部コーナー点（溝1 右側・アーク部のみ） ----
# groove_corners() の出力から x < nx - mn_straight（アーク部のみ）を残す
# Pt8（x=nx-mn_straight=1992）・Pt9（x=nx-1=1999）は直線部連続記録に統合するため除外
...
valid_arc  = corner_x_r < (nx - mn_straight)   # アーク部のみ（x < 1992）
corner_z_r = corner_z_r[valid_arc]
corner_x_r = corner_x_r[valid_arc]
print(f"T1 アーク部計測点数: {len(corner_z_r)}")
for i in range(len(corner_x_r)):
    print(f"  Pt{i+1}: (x={corner_x_r[i]}, z={corner_z_r[i]})")

# ---- [変更②] 直線部右壁の連続記録設定 ----
# x=[nx-mn_straight, ..., nx-1] = [1992, ..., 1999]（mn_straight=8 点）
# z=z_groove_end=2025（右壁、T13_isfree=solid）
_straight_x = np.arange(nx - mn_straight, nx, dtype=int)
_straight_z = z_groove_end
print(f"直線部右壁連続記録: x=[{_straight_x[0]}, {_straight_x[-1]}], z={_straight_z}  ({mn_straight} 点)")
...
_T1_straight  = np.zeros((mn_straight,       _t_rec_len), dtype=float)
...
_straight_x_cp = cp.array(_straight_x)
...
for t in range(int(t_max)):
    ...
    if _t_rec_start <= t < _t_rec_start + _t_rec_len:
        ...
        # [変更③] 直線部右壁の連続記録
        _T1_straight[:, ti]  = cp.asnumpy(T1[_straight_x_cp, _straight_z])
...
np.savez(
    ...
    # 直線部右壁連続記録
    T1_straight  = _T1_straight,
    straight_x   = np.array(_straight_x),
    straight_z   = np.array(_straight_z),
    mn_straight  = np.array(mn_straight),
    # T3 ギャップ連続記録
    T3_gap       = _T3_gap,
