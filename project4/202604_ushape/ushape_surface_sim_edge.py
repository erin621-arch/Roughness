...
cz2, cx2 = groove_corners(z2_start,       z2_end,       z2_center,       'left')
corner_z_r = np.array(cz1, dtype=int);  corner_x_r = np.array(cx1, dtype=int)
corner_z_l = np.array(cz2, dtype=int);  corner_x_l = np.array(cx2, dtype=int)

valid_r = (corner_x_r < nx) & (corner_x_r >= 0) & (corner_z_r >= 0) & (corner_z_r < nz)
valid_l = (corner_x_l < nx) & (corner_x_l >= 0) & (corner_z_l >= 0) & (corner_z_l < nz)
corner_z_r = corner_z_r[valid_r];  corner_x_r = corner_x_r[valid_r]
corner_z_l = corner_z_l[valid_l];  corner_x_l = corner_x_l[valid_l]
print(f"T1 計測点数: 右={len(corner_z_r)}, 左={len(corner_z_l)}")
...
_T1_corners_l = np.zeros((len(corner_z_l), _t_rec_len), dtype=float)
...
_corner_x_r_cp = cp.array(corner_x_r);  _corner_z_r_cp = cp.array(corner_z_r)
_corner_x_l_cp = cp.array(corner_x_l);  _corner_z_l_cp = cp.array(corner_z_l)
...
for t in range(int(t_max)):
    ...
    if _t_rec_start <= t < _t_rec_start + _t_rec_len:
        ...
        _T1_corners_l[:, ti] = cp.asnumpy(T1[_corner_x_l_cp, _corner_z_l_cp])
...
npz_path = os.path.join(
    output_dir,
    f"ushape_surface_map_edge_pitch{int(f_pitch*1e5)}_depth{int(f_depth*1e5)}.npz"
)
np.savez(
    npz_path,
    T1_corners_r = _T1_corners_r,
    T1_corners_l = _T1_corners_l,
    T3_gap       = _T3_gap,
    corner_x_r   = corner_x_r,
    corner_z_r   = corner_z_r,
    corner_x_l   = corner_x_l,
    corner_z_l   = corner_z_l,
