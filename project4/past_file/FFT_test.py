...
exp_gate_start = 0      # 実験データの切り出し開始探索位置
sim_gate_start = 7560   # シミュレーションの切り出し開始探索位置
...
if __name__ == "__main__":
    ...
    # --- シミュレーション CSV (Target: Rectangular Defect) ---
    file_sim = os.path.join(base_dir, "kukei1_ori_cupy_pitch125_depth10.csv")

    # --- 実験 CSV (Target: Rectangular Defect / scope_5b1_40~49) ---
    exp_files = [
        "scope_5b1_40.csv", "scope_5b1_41.csv", "scope_5b1_42.csv", "scope_5b1_43.csv", "scope_5b1_44.csv",
        "scope_5b1_45.csv", "scope_5b1_46.csv", "scope_5b1_47.csv", "scope_5b1_48.csv", "scope_5b1_49.csv",
    ]
    ...
    # 平滑面実験データ (Reference: Smooth / scope_5b1_0~9)
    exp_files_smooth = [
        "scope_5b1_0.csv", "scope_5b1_1.csv", "scope_5b1_2.csv", "scope_5b1_3.csv", "scope_5b1_4.csv",
        "scope_5b1_5.csv", "scope_5b1_6.csv", "scope_5b1_7.csv", "scope_5b1_8.csv", "scope_5b1_9.csv",
    ]

    print("--- ターゲット(Defect: Rectangular) ---")
    print("シミュレーション CSV :", os.path.basename(file_sim))
    print(f"実験 CSV : {len(exp_files)} files")
    
    print("--- 基準(Smooth: Rectangular base) ---")
    ...
    wave_sim_smooth_resampled = wave_sim_smooth_resampled * - 1.0
    '''
    ...
    # Smooth(平滑)：★ここを修正（安全のため別軸を作成）
    idx_exp_smooth = np.arange(len(wave_exp_smooth_mean))
    plt.plot(idx_exp_smooth, wave_exp_smooth_mean, label="experiment smooth", alpha=0.5, linestyle="--")
