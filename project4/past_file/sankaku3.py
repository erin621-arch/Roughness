...
def isfree(nx, ny, f_width, f_pitch, f_depth, mesh_length):
    ...
    for i in range(num_f):
        # ★修正：配置間隔を mn_period (実質のピッチ) に変更
        # y_start = mn_nf + i * mn_period  # すきまスタートの場合
        y_start = i * mn_period          # きずスタートの場合（図面の左端基準ならこちら）
        
        # ※図面では「左端から矢印が始まっている」ため、
        # x=nx, y=0 の地点は「きずの左端」である可能性が高いです。
        # もし実験が「すきま」から始まっているなら、y_start に mn_nf を足してください。

        if y_start >= ny:
            break
            
        # 以降の三角形描画ロジックは同じ（ただし y_end の計算に注意）
