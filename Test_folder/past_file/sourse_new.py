...
def import_data(pitch, depth):
    ...
    csv_files = glob.glob(os.path.join(f"C:\\Users\\Fujii Kotaro\\project2\\experience_new\\{pitch}_{depth}", "*.csv"))
    i = 0
...
def select_pitch_and_image(pitch, depth_list):
    sim_data_ref = np.loadtxt(r"C:\Users\Fujii Kotaro\project2\data_all\cupy_pitch125_depth0.csv")
    ...
    for i in range(len(depth_list)):
        sim_data[i] = np.loadtxt(f"C:\\Users\\Fujii Kotaro\\project2\\data_all\\cupy_pitch{pitch}_depth{depth_list[i]}.csv")
