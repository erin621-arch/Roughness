##########################################################################
#    - 三角きず FDTD + CSV保存 + アニメーション + 波形プロット
#    - sankaku3.py のピッチ定義 (p=W+gap+W) に修正済み
##########################################################################
...
def isfree(nx, ny, f_width, f_pitch, f_depth, mesh_length):
    ...
    # ★修正箇所：図面の定義 (p = W + すきま + W) に基づく計算
    # すきま = p - 2W
    mn_nf = max(0, mn_p_val - 2 * mn_w)
    
    # 実際の繰り返し間隔（Period） = W + すきま
    ...
    # ピッチ本数（繰り返し間隔 mn_period で割る）
    num_f = int(np.ceil(ny / mn_period))

    # 三角形の自由面をつくる
    for i in range(num_f):
        # ★修正：配置間隔を mn_period に変更し、開始位置を調整
        y_start = i * mn_period          # きずスタート（図面の左端基準）
...
# ログ用の計算もsankaku3に合わせて修正
mn_w = int(f_width / mesh_length)
mn_p_val = int(f_pitch / mesh_length) # mn_p -> mn_p_val
mn_nf = max(0, mn_p_val - 2 * mn_w)   # すきま
mn_period = mn_w + mn_nf              # 周期
mn_d = int(f_depth / mesh_length)

num_f = int(y_length / (mn_period * mesh_length)) # 概算本数
...
ani_path = os.path.join(
    output_dir,
    f"sankaku2_ani_pitch{int(f_pitch * 1e5)}_depth{int(f_depth * 1e5)}_animation.mp4"
)
