...
def process_one_case(exp_dir, shape, p_val, d_val, sim_base_dir):
    """
    Smoothデータを使わず、単体の正規化スペクトルを計算する関数
    """
    ...
    # ★ ここを変更：単体の最大値で正規化
    norm_single_exp = f_exp_tgt / np.max(f_exp_tgt)
    norm_single_sim = f_sim_tgt / np.max(f_sim_tgt)
    ...
    for shape_name, case_list in results_by_shape.items():
        ...
        for i, ax in enumerate(axes_flat):
            if i < num_plots:
                ...
                # 凡例ラベルを元のFFT.pyの「6. 単体スペクトル」に合わせて変更
                ax.plot(d['freq'], d['exp'], label="Exp (Norm)", color='tab:orange')
                ax.plot(d['freq'], d['sim'], label="Sim (Norm)", color='tab:blue')
        ...
        # 全体のタイトル
        main_title = f"[{shape_name}] Normalized Amplitude spectrum ({FREQ_MIN/1e6:.1f}-{FREQ_MAX/1e6:.1f} MHz)"
        fig.suptitle(main_title, fontsize=16)
        
        # ファイル名を変更
        save_name = f"3_Single_Spectrum_{shape_name}.png"
