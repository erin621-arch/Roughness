...
if __name__ == "__main__":
    ...
    for idx, fpath in enumerate(filenames):
        ...
        # =============================================================
        # ★★★ ラベル作成ロジックの修正部分 ★★★
        # depth12.5 や d12.5、12.5 などのパターンから数値を抽出して
        # "depth 0.125mm" の形式に変換します。
        # =============================================================
        try:
            # 拡張子を除去
            ...
            if match:
                ...
                # ラベル生成 (小数点以下3桁まで表示)
                custom_label = f"depth {depth_val:.3f}mm"
            else:
                # 数値が見つからない場合はファイル名をそのまま使う
                custom_label = fname

        except Exception as e:
            print(f"Label parsing error: {e}")
            custom_label = fname
        # =============================================================
        ...
        try:
            ...
            # 切り出し
            if len(wave_sim_resampled) > sim_gate_start:
                ...
                ax.set_title(f"Simulation: {custom_label}", loc='left', fontsize=12, fontweight='bold')
                ax.set_xlabel("Time [s]")
                ax.set_ylabel("Pressure [Pa]")
                # 必要に応じて保存やcloseを行ってください
                # plt.close(fig) 

                # B. 重ね書き用のグラフへのプロット追加
                ax_all.plot(t_rel_sim, -seg_sim, linewidth=1.5, label=custom_label)
            else:
                print(f"Warning: Data too short for {fname}")
