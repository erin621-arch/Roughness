...
このプログラムでは，縦方向応力$T_1$および横方向応力$T_3$を複数の観測点で離散的に記録する形にしました. 
Fig.~\ref{fig:meas_points}に，くさび形表面における$T_3$計測点の配置確認図．
...
矩形型についての論文(藤井先輩の去年の修論）のFig.~3.5に相当する図を再現するため，可視化プログラム\path{plot_kusabi_T1_T3_surface_map.py}を新たに作成しました．

Fig.~\ref{fig:T1T3_map}に，得られた図を示します．

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth, bb=0 0 791 567]{../../project4/tmp_output/kusabi_T1_T3_surface_map_pitch125_depth20.png}
  \caption{くさび形きずにおける$T_1$・$T_3$（pitch = 1.25 mm, depth = 0.20 mm）}
  \label{fig:T1T3_map}
\end{figure}
